...
function AdminLogin() {
  ...
  const onSubmitData = async (e) => {
    ...
    if (email === "") {
      ...
      toast.error("Password should be between 4 and 20 characters");
    } else {
      setLoder(true);
      ...
      try {
        ...
        if (response.ok) {
          localStorage.setItem("token-admin", JSON.stringify(data.token));
          localStorage.setItem("name-admin", JSON.stringify(data.name));
          localStorage.setItem("Admin-Id", JSON.stringify(data.AdminId));
          Navigater("/admin-page");
          // setLoder(false);
